...
<aside class="w-72 bg-slate-900 text-slate-300 min-h-screen sticky top-0 shadow-xl flex flex-col h-screen overflow-y-auto">
    <div class="p-8 flex-1">
        ...
        <nav class="space-y-2">
            ...
            <a href="{{ route('pembina.dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('pembina.dashboard') ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }} transition-all">
            ...
            <a href="{{ route('siswa.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('siswa.index') ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }} transition-all group">
                <svg class="w-5 h-5 {{ request()->routeIs('siswa.index') ? 'text-white' : 'text-slate-500 group-hover:text-blue-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">

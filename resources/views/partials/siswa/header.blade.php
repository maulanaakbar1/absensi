<nav class="bg-white/80 backdrop-blur-md sticky top-0 z-10 border-b border-slate-100 px-8 py-4 flex justify-between items-center">
    {{-- Sisi Kiri: Samain teks sambutannya --}}
    <div>
        <h1 class="text-sm font-medium text-slate-400">Selamat datang kembali,</h1>
        <p class="text-lg font-bold text-slate-800 italic">Portal Siswa</p>
    </div>

    <div class="flex items-center gap-6">
        {{-- Sisi Kanan: Profile Info --}}
        <div class="flex items-center gap-3 border-r border-slate-100 pr-6">
            <div class="text-right">
                {{-- Gunakan guard siswa --}}
                <p class="text-sm font-bold text-slate-700">{{ auth('siswa')->user()->name }}</p>
                <p class="text-[10px] text-emerald-500 font-bold uppercase">Online</p>
            </div>
            {{-- Avatar Bulat Sempurna ala Admin --}}
            <div class="w-10 h-10 rounded-full bg-slate-200 border-2 border-white shadow-sm flex items-center justify-center font-bold text-slate-500 uppercase">
                {{ substr(auth('siswa')->user()->name, 0, 1) }}
            </div>
        </div>

        {{-- Logout Button: Style Admin --}}

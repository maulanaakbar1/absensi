...
<div x-data="{ openModal: false, editMode: false, currentData: {} }" class="space-y-6">
    ...
    <div class="flex flex-col gap-4">
        ...
        {{-- Container tombol: default (mobile) ke kiri, md (desktop) ke kanan --}}
        ...
        <form action="{{ route('pembina.anggota.index') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-end">
            ...
            <div class="flex-1 w-full">
                ...
                <input 
                    ...
                    class="w-full mt-1 px-4 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-0 transition text-sm"
                >
            </div>

            {{-- Filter Tahun --}}
            <div class="w-full md:w-48">
                <label class="text-xs font-bold text-slate-400 uppercase ml-1">Tahun Angkatan</label>
                <select 
                    name="tahun" 
                    class="w-full mt-1 px-4 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-0 transition text-sm"
                >
            ...
            <div class="flex gap-2 w-full md:w-auto">
                <button type="submit" class="bg-slate-800 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-slate-900 transition text-sm flex-1">
                    Filter
                </button>
                @if(request('search') || request('tahun'))
                    <a href="{{ route('pembina.anggota.index') }}" class="bg-slate-100 text-slate-600 px-6 py-2.5 rounded-xl font-bold hover:bg-slate-200 transition text-sm flex-1 text-center">
                        Reset
                    </a>
                @endif
            </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50/50 border-b border-slate-100">
                    ...
                    <th class="px-6 py-4 text-xs font-bold text-slate-400 uppercase text-center">Tahun Angkatan</th>
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-6 py-4 text-sm font-bold text-slate-600">{{ $s->nisn }}</td>
                    <td class="px-6 py-4 text-sm font-bold text-slate-600">
                        <span class="text-emerald-500 uppercase">{{ $s->kelas }}</span>
                    </td>
                    <td class="px-6 py-4 text-sm font-bold text-slate-600 text-center">{{ $s->tahun_angkatan }}</td>
                    <td class="px-6 py-4">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="nis" x-model="currentData.nis" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-emerald-500" required>
                            </div>
                            <div>
                        ...
                        <div>
                            <label class="text-xs font-bold text-slate-400 uppercase ml-1">Tahun Angkatan</label>
                            <input type="number" name="tahun_angkatan" x-model="currentData.tahun_angkatan" placeholder="2025" min="2000" max="2099" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-0 transition" required>
                        </div>
                        <div>
